...
class CustomerService {
  ...
  async SignIn(userInputs) {
    ...
    try {
      ...
      if (existingCustomer) {
        const vaildPassword = await ValidatePassword(
          password,
  ...
  async SignUp(userInputs) {
    ...
    try {
      ...
      console.log("New Token" + userPassword);
  ...
  async ManageCart(customerId, product, qty, isRemove) {
    console.log("Inside ManageCart - service");
    try {
  ...
  async SubscribeEvents(payload) {
    ...
    switch (event) {
      ...
      case "REMOVE_FROM_WISHLIST":
        this.AddToWishlist(userId, prodcut);
        break;
      case "ADD_TO_CART":
